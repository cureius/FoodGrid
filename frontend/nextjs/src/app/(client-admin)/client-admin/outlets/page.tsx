...
import { createOutlet, deleteOutlet, listOutlets, updateOutlet, type OutletUpsertInput } from "@/lib/api/clientAdmin";
...
export default function Page() {
  ...
  const canSubmit = useMemo(() => !!form.name.trim() && !!form.timezone.trim(), [form.name, form.timezone]);
    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ border: "1px solid #ddd", padding: 12, borderRadius: 8, maxWidth: 720 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <input placeholder="Outlet name" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} />
          <input
              ...
              {outlets.map((o) => (
                <tr key={o.id}>
                  ...
                  <td style={{ borderBottom: "1px solid #eee", padding: 8, textAlign: "right" }}>
                    <button onClick={() => onQuickUpdate(o)} disabled={saving}>Edit</button>
                    <button onClick={() => onDelete(o.id)} disabled={saving} style={{ marginLeft: 8 }}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
